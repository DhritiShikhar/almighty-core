...
// TestIterationInTimeframe tests iteration should be active when it is in timeframe
func (rest *TestIterationREST) TestIterationInTimeframe() {
	// given
	_, _, _, _, itr1 := createSpaceAndRootAreaAndIterations(rest.T(), rest.db)
	assert.Equal(rest.T(), iteration.IterationActive, itr1.ActiveStatus) // iteration falls in timeframe, so iteration is active
}

// TestIterationOutsideTimeframe tests iteration should not be active when it is outside the timeframe
func (rest *TestIterationREST) TestIterationOutsideTimeframe() {
	sp, _, _, _, itr1 := createSpaceAndRootAreaAndIterations(rest.T(), rest.db)
	assert.Equal(rest.T(), iteration.IterationActive, itr1.ActiveStatus) // iteration falls in timeframe, so iteration is active
