...
func (rest *TestTrackerREST) TestCreateTracker() {
	...
	payload := app.CreateTrackerPayload{
		Data: &app.Tracker{
			...
			Type: remoteworkitem.APIStringTypeTracker,
		},
	}
...
func (rest *TestTrackerREST) TestCreateTrackerValidId() {
	...
	if created != nil && created.Data.ID != tracker.Data.ID {
		t.Error("Failed because fetched Tracker not same as requested. Found: ", tracker.Data.ID, " Expected, ", created.Data.ID)
	}
}
