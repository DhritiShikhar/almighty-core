...
func (s *TestSpaceTrackerQueries) TestListSpaceTrackerQueriesOK() {
	...
	_, tq2 := test.CreateTrackerqueryCreated(s.T(), svc.Context, svc, trackerQueryCtrl, &tqpayload1)
	require.NotNil(s.T(), tq2)

	// Create one TrackerQuery from second space
	tqpayload2 := newCreateTrackerQueryPayload(fxt.Spaces[1].ID, fxt.Trackers[0].ID, fxt.WorkItemTypes[0].ID)

	_, tq3 := test.CreateTrackerqueryCreated(s.T(), svc.Context, svc, trackerQueryCtrl, &tqpayload2)
